...
def main(word):
    ...
    articles = newsapi.get_everything(q=word,  
    ...
    '''
    As I learned in AQM-2000, Data preprocesses is essential in various data-mining as it help to clear the noises of data, clean and prepare the text data for analysis.
    ...
    nltk.download('stopwords')  # Stopwords. These are the words that does not add significant value to the text, such as "a", "an" and "the". Removing them will help the program to analyze text more efficiently
    ...
    def preprocess(text):
        ...
        stopwords_set = nltk.corpus.stopwords.words("english")  # this gets a set of English stopwords from NLTK
        tokens = [word for word in tokens if word not in stopwords_set]  # We use the list above to filter out stopwords
        tokens = [word for word in tokens if word.isalpha()]  # Remove punctuation and numbers
        tokens = [lemmatizer.lemmatize(word) for word in tokens]  # This line applies lemmatization to each token which converts them to its dictionary form that is better for analyzing
        return ' '.join(tokens)  # Joins the processed tokens back into a single string with spaces between words and returns it.

    preprocessed_articles = []
    for article in articles['articles']:  # I was getting empty result before adding '['articles']' to this line. It was suggested by ChatGpt as it ensure that this match to my data structure which I ignored
        text_to_preprocess = article['description']
        if text_to_preprocess:  # check whether article is not None or an empty string or not
    ...
    '''
    As sklearn isnot suggested to use in this project, I will be calculating TF-IDF manually. TF-IDF is a statistical measure that evaluates how relevant a word is to a document in a collection of documents. Hence, it is very important to this project.
'''
    # I copied how to calculate TF-IDF from this website with some alteration so it fits my code:https://www.codingthepast.com/2023/04/26/Text-Mining-in-Python.html
    ...
    def calculate_idf(documents):
        ...
        for word in all_words:
            ...
            idf_dict[word] = math.log(N / containing_docs + 1)  # Added 1 to avoid division by zero
